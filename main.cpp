...
public:
    ...
    vector<Document> AddFindRequest(const string& raw_query, DocumentPredicate document_predicate) {
        ...
        result.success = result.documents.empty() ? false : true;
    ...
    vector<Document> AddFindRequest(const string& raw_query, DocumentStatus status) {
        ...
        QueryResult result;
        result.documents = search_server_.FindTopDocuments(raw_query, status);
        result.success = result.documents.empty() ? false : true;
    ...
    vector<Document> AddFindRequest(const string& raw_query) {
        ...
        QueryResult result;
        result.documents = search_server_.FindTopDocuments(raw_query);
        result.success = result.documents.empty() ? false : true;
...
private:
    ...
    int nrequests_;
    int nempty_requests_;

private:
    void Update() {
        ++nrequests_;
        while (requests_.size() >= min_in_day_) {
            QueryResult qr = requests_.back();
            if (!qr.success) {
                --nempty_requests_;
            }
            requests_.pop_back();
    ...
    void Add(const QueryResult& qr) {
        requests_.push_front(qr);
...
int main() {
    ...
    cout << "1439 empty requests: "s << request_queue.GetNoResultRequests() << endl;
    ...
    cout << "1438 empty requests: "s << request_queue.GetNoResultRequests() << endl;
